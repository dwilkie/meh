Feature: Payment Agreement
  ...
  As a seller
  I want to be able set up payment agreements to pay suppliers when a product order is created or after the supplier accepts or completes a product order
  ...
  Background:
    ...
    And a product order exists for the product with quantity: 4
  ...
  Scenario Outline: I have set up a payment agreement with my supplier
    ...
    And the product order <is_not_yet_or_was_already> accepted

    When the supplier <processes> the product order

    Then a supplier payment should exist with product_order_id: the product order, cents: "920000", currency: "THB", seller_id: the seller, supplier_id: the supplier
  ...
  Scenario: I have set up a payment agreement for when a product order is created
    ...
    When a product order is created for the product with quantity: 1

    Then a supplier payment should exist with product_order_id: the product order, cents: "1200", currency: "THB", seller_id: the seller, supplier_id: the supplier
    And the most recent job in the queue should be to send the supplier payment
  ...
  Scenario Outline: I have set up a payment agreement with my supplier but the product's supplier price is zero
    ...
    And a product order exists for the product with quantity: 4
    And the product order is not yet accepted

    When the supplier accepts the product order
  ...
  Scenario: I have set up a payment agreement with my supplier and I also set up another payment agreement for a product which is not in this order
    ...
    And the product order was already accepted

    When the supplier completes the product order
  ...
  Scenario: I have set up a payment agreement with another supplier who is not the supplier for this order
    ...
    When the supplier: "Fon" accepts the product order
  ...
  Scenario: I am also the supplier for this order
    ...
    When I accept the product order

    Then a supplier payment should not exist
    And the most recent job in the queue should not be to send the supplier payment

  Scenario: I have not yet set up any payment agreements
    Given the product order is not yet accepted
