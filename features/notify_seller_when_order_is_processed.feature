Feature: Notify seller when an order for a product they are selling is processed
  ...
  Background:
    ...
    And a product exists with seller: the seller, supplier: the supplier, item_number: "567864ab"
  ...
  Scenario Outline: Notify seller when an order is processed by a supplier
    Given a supplier_order exists with id: 154671, product: the product, status: "<status>", seller_order: the seller order, quantity: 5
