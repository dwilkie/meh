Feature: Notify seller when an order for a product they are selling is processed
  ...
  Background:
    ...
    And a mobile_number exists with number: "66354668789", password: "1234"
  ...
  Scenario Outline: Notify seller when an order is processed by a supplier
    ...
    And the outgoing_text_message should be a translation of "supplier processed seller's order" in "en" (English) where seller: "Dave", supplier: "Bruno", supplier_contact_details: "+66354668789", process: "<processed>", seller_order_number: "154670", product_code: "567864ab"
    And the outgoing_text_message should include a translation of "<processed>" in "en" (English)
